...
def equal_length_chunks(l1, l2, l3, l4):
    """
    ...
    for i in range(len(l1)):
        if len(l1[i]) != prev:
            if prev != -1:
                yield l1[start:end+1], l2[start:end+1], l3[start:end+1], l4[start:end+1]
            start = i
            end = i
        else:
            end += 1
...
def load_data(file, args, word2id, prod2id):
    ...
    for line in f:
        if not first_line:
            if len(line.strip()) != 0:
                segs, orig, ids, total = line_to_words(line, 0, 10000,
    ...
    return data, products, scodes, original
...
def main():
    ...
    data, products, scodes, original = load_data(args.data, args, word2id, prod2id)
    ...
    with h5py.File(filename, 'w') as f:
        for i, (segments, prods, codes, segs_o), in enumerate(equal_length_chunks(data, products, scodes, original)):
            max_len_batch = len(max(segments, key=len))
            batch_id = str(i)

            for j in range(len(segments)):
                segments[j].extend([0] * (max_len_batch - len(segments[j])))
            # data format: [array([622,  29], dtype=int32),array([   2, 2768], dtype=int32)]
            f['data/' + batch_id] = np.array(segments, dtype=np.int32)
            # products format: [1842, 1655] product id in the product mapping file
            f['products/' + batch_id] = np.array(prods,  dtype=np.int32)
            # scodes format: [b'B0064RDIWU-0086-0001', b'B00307F8PW-0043-0019'] contains product-id-index
            f.create_dataset('scodes/' + batch_id, (len(codes),), dtype="S{0}".format(100), data=codes)
            dt = h5py.special_dtype(vlen=bytes)
            # original format: [b'I love this cover.', b'and looks cute.']
