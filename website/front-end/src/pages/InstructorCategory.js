...
import HoverScatter from "./@d3/InstructorHoverScatter";
...
const InstructorCategory = () =>{
    ...
    return word[0].toUpperCase() + word.substring(1);
      ...
      summary.pop()
    const allComments = JSON.parse(sessionStorage.getItem("instructorScatter"))
    // get full comments corresponding to summary
    let commentList = []
    summary.forEach(sent =>{
        console.log(Array.from(allComments).filter(item => item.comments === sent))
    ...
    let tableData = JSON.parse(sessionStorage.getItem("instructorScatter")).filter(item => item.category==aspect)
