...
const Bubble = () =>{
    ...
    React.useEffect(() =>{
        if(sessionStorage.getItem("instructorBubble")!=null){
            let bubbleData = JSON.parse(sessionStorage.getItem("instructorBubble"))
            setLoading(false)
            generateChart(bubbleData, dimensions.width, dimensions.height)
        }else{
            axios({
              method: 'get',
              url: `${API}/Bubbles/instructor-${cid}`,
            }).then(function (response) {
                sessionStorage.setItem("instructorBubble", JSON.stringify(response.data.data))
                setLoading(false)
            }).catch((error) =>{
                console.log(error)
            })
