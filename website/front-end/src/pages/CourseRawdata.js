...
import load from "@observablehq/runtime/src/load";
...
const CourseRawdata = () =>{
    ...
    React.useEffect(() =>{
        if(sessionStorage.getItem("courseRawData")==null){
            axios({
              method: 'get',
              url: `${API}/Tables/course-${cid}`,
                ...
                sessionStorage.setItem("courseRawData", JSON.stringify(response.data.data))
            ...
            let tableData = JSON.parse(sessionStorage.getItem("courseRawData"))
            ...
            <div className={"background-block"}>
                ...
                <div className={"row-block"}>
                    <div className={"sub-block"} style={{width: "-webkit-fill-available"}}>
                        ...
                        {/*version 2*/}
                        {loading? <div></div>:Array.from(reformatComments).map((item, i)=>(<RawTable key={"courseTable"+i} title={i} data={item}></RawTable>))}
